...
class BJNationEnterprise(object):
    def __init__(self):
        chrome_option = webdriver.ChromeOptions()
        chrome_option.add_experimental_option('excludeSwitches', ['enable-automation']) # 设置为开发者模式启动
        executable_path = r'C:\Users\Administrator\AppData\Local\Google\Chrome\Application\chromedriver.exe'
        self.driver = webdriver.Chrome(executable_path=executable_path, chrome_options=chrome_option)
        self.driver.set_window_size(1440, 900)
        # executable_path = r"D:\Python\Python36\Scripts\msedgedriver.exe"
        # self.driver = webdriver.Edge(executable_path=executable_path)
        # self.driver.set_window_size(1440, 900)

    def visit_index(self):
        # 输入查询的企业
        self.driver.get("http://bj.gsxt.gov.cn/index.html")
        search_word = WebDriverWait(self.driver, 20).until(EC.presence_of_element_located((By.NAME, 'search_form')))
        search_word.clear()
        search_word.send_keys("中国移动")

        # 点击查询，弹出滑块验证码
        query_btn = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.ID, 'btn_query')))
        query_btn.click()
